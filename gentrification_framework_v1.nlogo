...
to setup
  ...
  set my-list [0 1 2 3 4] ;GOTO
  foreach my-list
  [
    x -> set my-list (replace-item x my-list (patches with [(pycor = -1 - (2 * x)) and (pxcor >= 1 and pxcor <= (neighbourhood-capacity))]) )
    ask item x my-list
    [
      set pcolor gray + 4
      if (pxcor <= (neighbourhood-population))
      [
        sprout 1
        [
